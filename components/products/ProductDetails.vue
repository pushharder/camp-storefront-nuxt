    ...
    <section class="grid-in-left-bottom md:mt-8">
      ...
      <div v-if="selectedProduct?.sizes.length" class="px-4">
        ...
        <ul class="grid grid-cols-5 gap-2 py-2">
          <li v-for="size in selectedProduct.sizes" :key="size.value">
            <SfChip
              :class="[
                'w-full',
                {
                  'font-bold': size.value === productSize?.value,
                  'bg-primary-100': size.value === selectedSize?.value,
                },
              ]"
              size="sm"
          ...
          <li v-for="color in availableColors" :key="color.value">
            <SfListItem
              ...
              :class="[
                ...
                { 'font-bold': color.value === selectedColor?.value },
              ]"
              :selected="color.value === selectedColor?.value"
              @click="updateColor(color)"
            >
              <template #prefix>
                <input :value="color.value" class="appearance-none peer" type="checkbox" />
                <span
                  ...
                  :class="{ 'bg-primary-100': color.value === selectedColor?.value }"
                  ><SfThumbnail
                    class="w-6 h-6"
                    size="sm"
...
<script setup lang="ts">
  ...
  const selectedSize = ref<ProductDetailsAttributeDTO | null>(null)
  const selectedColor = ref<ProductDetailsAttributeDTO | null>(null)

  const selectedProduct = computed(() => productStore.selectedProduct)

  const productImages = computed(() => selectedProduct.value?.images || [])
  const productSize = computed(() => selectedProduct.value?.size || null)
  const productColor = computed(() => selectedProduct.value?.color || null)

  watch(productSize, value => (selectedSize.value = value))
  watch(productColor, value => (selectedColor.value = value))

  watch(selectedColor, () => {
    router.push(`/products/${selectedProduct.value?.code}-${selectedSize.value?.label}-${selectedColor.value?.label}`)
  })
  watch(selectedSize, () => {
    router.push(`/products/${selectedProduct.value?.code}-${selectedSize.value?.label}-${selectedColor.value?.label}`)
  })
  ...
  const updateSize = (size: ProductDetailsAttributeDTO) => {
    selectedSize.value = size
  }

  const updateColor = (color: ProductDetailsAttributeDTO) => {
